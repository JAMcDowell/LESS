...
if IN.RUN.SPATIAL
    ...
    for s = 1:size(SITES,2)                                                 % For each site...
        %% Site Positions
        PATHFIND.SPATIAL.SITES(s).Sites_Name = SITES(s).Name;               % Extract Site name.
...
if IN.RUN.DISCRETE && IN.RUN.SPATIAL                                        % If SPATIAL bathy data is available for any type of simulation...        
    ...
    for s = 1:size(PATHFIND.DISCRETE.SITES,2)
        ...
        disp(' - Finding A* shortest node paths from Sites to landfall for Transmission...');
        Start = tic;
       ...
       [PATHFIND.DISCRETE.SITES(s).TRANSMISSION.AStar_OptimalPath2LandFall_mn,...
        ...
        disp(' - Finding A* shortest node paths from Site to Port for Operations...');
        Start = tic;
