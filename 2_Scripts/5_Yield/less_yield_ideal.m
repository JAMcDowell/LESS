...
% This function calculates Ideal Yield (Energy Generated) for Discrete 
% Sites and/or the entire Spatial Region for each Device.
...
if IN.RUN.DISCRETE
    ...
    for s = 1:size(SITES,2) 
        % Start Timer
        Start = tic; disp([' - Calculating Ideal Energy Generated for "',char(string(SITES(s).Name)),'" site...']);
    ...
    for d = 1:size(DEVICES,2)                                               % For each Device...
        ...
        for m = 1:12                                                        % For each month...                 
            ...
            YIELD.SPATIAL.IDEAL.GENERATED.DEVICES(d).Monthly(m).Energy_Ideal_GenPerDevice_kWh...
                = single(squeeze(sum(YIELD.SPATIAL.IDEAL.GENERATED.DEVICES(d).Monthly(m).Power_Ideal_GenPerDevice_kW,1)));
